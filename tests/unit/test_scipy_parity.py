...
class TestPeakProminencesParity:
    ...
    def test_random_signals(self, seed):
        ...
        scipy_proms, scipy_left, scipy_right = scipy_peak_prominences(signal, scipy_peaks)
        jax_proms, jax_left, jax_right = peak_prominences(signal, scipy_peaks)

        np.testing.assert_allclose(
            np.asarray(jax_proms), scipy_proms, rtol=EXACT_RTOL,
    ...
    def test_wlen_parameter(self, wlen):
        ...
        np.testing.assert_allclose(
            np.asarray(jax_result[0]), scipy_result[0], rtol=EXACT_RTOL,
...
class TestPeakWidthsParity:
    ...
    def test_rel_height_values(self, rel_height):
        ...
        np.testing.assert_allclose(
            np.asarray(jax_result[1]), scipy_result[1], rtol=EXACT_RTOL,
            err_msg=f"Width heights differ at rel_height={rel_height}"
        )
        np.testing.assert_allclose(
            np.asarray(jax_result[2]), scipy_result[2], rtol=EXACT_RTOL,
            err_msg=f"Left IPs differ at rel_height={rel_height}"
        )
        np.testing.assert_allclose(
            np.asarray(jax_result[3]), scipy_result[3], rtol=EXACT_RTOL,
...
class TestInterpLinearParity:
    ...
    def test_extrapolation_handling(self):
        ...
        x = np.array([0., 1., 2., 3., 4.], dtype=np.float64)
        y = np.array([0., 1., 4., 9., 16.], dtype=np.float64)
        x_new = np.array([-1., 0.5, 2.5, 5.], dtype=np.float64)
...
class TestInterpCubicParity:
    ...
    def test_basic_parity(self):
        ...
        x = np.array([0., 1., 2., 3., 4., 5., 6., 7.], dtype=np.float64)
        y = np.array([0., 1., 4., 9., 16., 25., 36., 49.], dtype=np.float64)
        ...
        np.testing.assert_allclose(
            np.asarray(jax_result), scipy_result, rtol=CUBIC_RTOL,
...
class TestSavgolFilterParity:
    ...
    def test_various_parameters(self, window_length, polyorder):
        ...
        scipy_result = scipy_savgol(signal, window_length=window_length, polyorder=polyorder)
        jax_result = savgol_filter(signal, window_length=window_length, polyorder=polyorder)
    ...
    def test_preserves_peaks(self):
        ...
        np.testing.assert_array_equal(
            np.asarray(filtered_peaks), original_peaks,
